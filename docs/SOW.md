...
- **DEC-027 (2026-06-14): Provision rate-limiting to protect the demo under public traffic.** Context: the /demo and splash provisioning path creates a fresh seeded workspace per visitor and has no rate limit; once the posts and the demo link circulate, and through the judging window, a single actor could spam provisioning, exhausting DSQL writes and free-tier DPU and bloating the workspace set, and the "$0.00 this month" usage story depends on this not happening; this pulls the provision rate-limit forward from the post-judging follow-up list. Decision: provisioning is rate-limited per client IP using a sliding window tracked in DSQL, enforced before any workspace is created; over the limit returns a clean 429 with a retry hint and provisions nothing; the limit is a soft abuse-mitigation control, not a hard security boundary; the rate-limit read and write use the same chaos-immune healthy path as provisioning (DEC-025), so a drill never breaks it; the e2e suite uses an env-gated bypass so tests are not throttled. References: refines DEC-025 (chaos-immune provisioning); related to the deferred 7-day workspace sweep.
- **DEC-028 (2026-06-14): Looser provision limits sized for shared judge IPs, and no dead ends in provisioning.** Context: the provision rate limit is too tight for the judging reality and can strand a user; multiple judges commonly share one egress IP (corporate NAT, VPN, an Amazon office network), so a per-IP cap of 5 per 10 minutes would block the whole group after a few demo opens, the legitimate-traffic failure the control was meant to avoid; separately, a blocked or cancelled create dead-ends the user, from the switch screen a create that hits the limit, or the "return to workspace" control, lands on the splash instead of the active workspace, while joining by a saved link code correctly still works because it does not provision. Decision: bias the limit toward usability and make abuse the only thing it stops; raise the per-IP allowance so a shared-IP group of judges never trips it, enable a generous global ceiling as the real abuse and cost stop, and limit only the act of provisioning a new workspace, joining or switching to an existing workspace is never limited; guarantee no dead ends, every blocked or cancelled provisioning path returns the user to their current workspace when they have one, and the over-limit screen always offers a way forward. References: refines DEC-027 (provision rate-limiting) and DEC-025 (chaos-immune provisioning); supersedes nothing.
- **DEC-029 (2026-06-14): Adopt the v0-generated restyle across the app (presentational).** Context: the app was restyled in v0 (hackathon account) to the dark ops-terminal look across the splash, war room, Reliability surface, and incident detail. This adopts that restyle wholesale under the DEC-024 Part B adoption rule: presentational surfaces are taken as-is, and the stateful proof and status components are restyled with their logic, claim copy, chaos-state awareness (DEC-023/026), and every measured number verified byte-for-byte unchanged. The originally-scoped splash-only decision was drafted before the v0 pass finished; the full restyle was requested. Decision: adopt the v0 markup and styles for the splash (Onboarding plus a new inline Logo mark), the global theme (layout self-hosted Inter and JetBrains Mono web fonts, and globals.css: a faint app-wide radial background, focus-visible rings, font-smoothing, and a live-serving pulse), and the in-product surfaces (StatusBand, WorkspaceHeader, ControlPlanePanel, VerificationProofs, badges, the incident-detail timeline). Every change is presentational, reusing the existing palette tokens: no provisioning logic, rate-limit behavior (DEC-027/028), /demo behavior (DEC-025), API route, proof claim, or measured number changes, and the WarmUp pre-warm (DEC-024) and the anchor-arrival pulse (DEC-026) are preserved. No new external images, only self-hosted web fonts and an inline SVG logo. References: refines DEC-024 (Part B adoption rule) and docs/v0-prompt.md; preserves DEC-025/026/027/028 behavior; supersedes nothing.
...
- 2026-06-14: Added DEC-027 (provision rate-limiting to protect the demo under public traffic): /demo and the splash create path are rate-limited per client-IP hash (HMAC, salt in env) via a sliding window in a dedicated DSQL rate_limit table (migration 0009), enforced before provisioning on the chaos-immune healthy path (DEC-025); over the limit returns a clean 429 (an on-brand page for /demo, JSON for the create POST) and provisions nothing. Defaults window 600s / max 5 per IP, tunable via env, with an optional global backstop; the monitor prunes rows older than 24h; the e2e suite bypasses via an env-gated header. Refines DEC-025; moves the provision rate-limit off the follow-up list.
- 2026-06-14: Added DEC-028 (looser provision limits sized for shared judge IPs, and no dead ends in provisioning): the per-IP allowance is raised to 100 per 600s and a generous global ceiling (60 per minute) is enabled by default as the real abuse and cost stop; only provisioning a new workspace is limited, joining by code and switching to an existing workspace never are; the switch flow no longer clears the session before a new workspace is entered, so back, cancel, and return-to-workspace land in the current workspace, and the over-limit response renders in place with return-to-workspace, join-by-code, and a retry hint instead of stranding on the splash. Refines DEC-027 + DEC-025.
- 2026-06-14: Added DEC-029 (adopt the v0-generated restyle across the app, presentational): the dark ops-terminal restyle is adopted wholesale under the DEC-024 Part B rule for the splash (Onboarding + a new inline Logo), the global theme (self-hosted Inter/JetBrains Mono, an app-wide radial background, focus rings, a live-serving pulse), and the war-room/Reliability/incident surfaces (StatusBand, WorkspaceHeader, ControlPlanePanel, VerificationProofs, badges, incident timeline). Presentational only, reusing existing palette tokens: every proof claim string, measured number, and chaos-state step-aside (DEC-026) is byte-identical, WarmUp (DEC-024) and the anchor pulse (DEC-026) preserved, no API/route/rate-limit/provisioning change. Refines DEC-024.
